...
只输出一个 JSON 对象，不要输出 Markdown、代码围栏或额外解释。JSON 必须严格使用以下字段：
{
  ...
  "issues": ["逐项扣分点。每条必须写成：扣分点N｜类别｜严重度｜定位：原文/学生译文对应片段｜问题：具体错在哪里｜扣分原因：为什么影响得分｜修改方向：如何改。"],
  "suggestions": ["可执行的修改建议。每条对应一类高频问题，说明修改策略和检查方法。"]
}

要求：
1. issues 必须尽量覆盖学生译文中所有可扣分点，一条只写一个扣分点；如果问题较多，输出 6-12 条；如果问题较少，也至少输出 4 条，除非译文几乎没有可扣分点；
...
5. suggestions 输出 4-8 条，内容必须与 issues 中的具体扣分点对应，避免泛泛而谈；
6. 不要编造原文没有的信息；
7. score 必须是 0-100 的整数，按照 CATTI 二级笔译实务评分口径估分：重点看信息准确完整、术语和专名处理、逻辑衔接、语体风格、中文/英文表达自然度、语法标点与格式规范；60 分视为基本合格线；
8. 分数要偏严格，明显误译、漏译或语体失当应显著扣分；
9. 不要输出示范译文或完整改译，只能给局部修改方向；
10. 练习类型仅作语体判断参考；
11. JSON 数组里的每个字符串都要是单行文本，不要在字符串内部换行。
