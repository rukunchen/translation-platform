...
当前 `supabase/` 目录是历史 migration 集合，不是完整的从零初始化脚本。现有代码实际依赖这些基础表：
...
但当前仓库还缺少 `projects`、`documents`、`glossary_terms` 的基础创建 migration。新 Supabase 环境从零部署时，必须先补齐或执行这三张表的基础 schema，再执行后续 migration。

建议运行顺序：

1. 基础 schema：创建 `projects`、`documents`、`glossary_terms`。
...
`supabase/add_segments_column.sql` 是旧版 JSONB 分句结构的历史脚本，只在需要把旧 `documents.segments` JSONB 数据迁移到独立 `segments` 表时使用。全新环境通常不需要单独执行它。
...
先确认是否已经创建 `projects`、`documents`、`glossary_terms` 基础表。当前仓库中的后续 migration 会引用这些表，但没有包含它们的初始创建脚本。
