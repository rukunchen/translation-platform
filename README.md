...
- `frontier_reading_sessions`
- `frontier_reading_notes`
- `writing_literature_sources`
- `frontier_field_subscriptions`
- `frontier_literature_candidates`
- `frontier_literature_seed_status`
...
28. `supabase/28_frontier_ai_card_fields.sql`
29. `supabase/29_frontier_reading_sessions.sql`
30. `supabase/30_writing_literature_sources.sql`
31. `supabase/31_frontier_subscriptions_candidates.sql`
32. `supabase/32_frontier_literature_user_scope.sql`
33. `supabase/33_frontier_seed_library.sql`
...
确认只按 `01-33` 主链执行，不要执行 `supabase/archive/add_segments_column.sql`。如果使用 Supabase Dashboard 手动执行 SQL，请严格按 README 中的顺序执行。
